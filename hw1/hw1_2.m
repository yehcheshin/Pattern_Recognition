...
S1=[5 3;3 4];%covariance matrices S1
m2=[14;7];%mean vector:m2
S2=[5 3;3 4];%covariance matrices S2
m3=[16;1];%mean vector:m3
S3=[5 3;3 4];%covariance matrices S3
...
%data(X2 data): Nx2 :N筆1x2維的data座標
%y_label: 1xN :N筆data之grount truth label
X2 = [data1;data2;data3];
...
bayesian_g1 = Bayesian_classifier(m1,S1,P,X2);
bayesian_g2 = Bayesian_classifier(m2,S2,P,X2);
bayesian_g3 = Bayesian_classifier(m3,S3,P,X2);
bayesian_g = [bayesian_g1;bayesian_g2;bayesian_g3];

%3個classes Mahalanobis classifier's discrimintation function 結果
Mahalanobis_g1 = Mahalanobis_classifier(m1,S1,P,X2);
Mahalanobis_g2 = Mahalanobis_classifier(m2,S2,P,X2);
Mahalanobis_g3 = Mahalanobis_classifier(m3,S3,P,X2);
Mahalanobis_g = [Mahalanobis_g1;Mahalanobis_g2;Mahalanobis_g3];

%3個classes Euclidean classifier's discrimintation function 結果
Euclidean_g1 =  Euclidean_classifier(m1,S1,P,X2);
Euclidean_g2 =  Euclidean_classifier(m2,S2,P,X2);
Euclidean_g3 =  Euclidean_classifier(m3,S3,P,X2);
...
for i=1:n
   ...
   g(i) = -1/2*(x(i,:)*inv(S)*x(i,:)' - 2*m'*inv(S)*x(i,:)'+m'*inv(S)*m)+log(prior);
end
end
