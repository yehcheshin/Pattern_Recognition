...
S1=[4 0;0 4];%covariance matrices S1
m2=[12;8];%mean vector:m2
S2=[4 0;0 4];%covariance matrices S2
m3=[16;1];%mean vector:m3
S3=[4 0;0 4];%covariance matrices S3
...
%data(X1 data): Nx2 :N筆1x2維的data座標
...
bayesian_g1 = Bayesian_classifier(m1,S1,P,X1);
bayesian_g2 = Bayesian_classifier(m2,S2,P,X1);
bayesian_g3 = Bayesian_classifier(m3,S3,P,X1);
bayesian_g = [bayesian_g1;bayesian_g2;bayesian_g3];

%3個classes Mahalanobis classifier's discrimintation function 結果
Mahalanobis_g1 = Mahalanobis_classifier(m1,S1,P,X1);
Mahalanobis_g2 = Mahalanobis_classifier(m2,S2,P,X1);
Mahalanobis_g3 = Mahalanobis_classifier(m3,S3,P,X1);
Mahalanobis_g = [Mahalanobis_g1;Mahalanobis_g2;Mahalanobis_g3];

%3個classes Euclidean classifier's discrimintation function 結果
Euclidean_g1 =  Euclidean_classifier(m1,S1,P,X1);
Euclidean_g2 =  Euclidean_classifier(m2,S2,P,X1);
Euclidean_g3 =  Euclidean_classifier(m3,S3,P,X1);
...
% ret = drawBayesGauss2D(mu,c,prob,ax);

% for i=1:N 
%     if bayesian_pred(i)==1
%         plot(X1(i,1),X1(i, 2),'y+');
%         
%     elseif bayesian_pred(i)==2
%         plot(X1(i,1),X1(i,2),'m+');
%        
%     else bayesian_pred(i)==3
%         plot(X1(i,1),X1(i,2),'k+');
%        
%     end
%      hold on;
% end
plot_(X1,Euclidean_pred);
